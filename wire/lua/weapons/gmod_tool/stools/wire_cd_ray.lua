...
TOOL.ClientConVar[ "raymodel" ] = "models/props_junk/metal_paintcan001a.mdl"
...
function TOOL:LeftClick(trace)
	...
	local wire_cd_ray = MakeWireCDRay( ply, trace.HitPos, range, defZero, model, Ang )
...
function TOOL:RightClick(trace)
	...
	local range = self:GetClientNumber("Range")
	local defZero = (self:GetClientNumber("DefaultZero") ~= 0)
	local model = self:GetClientInfo("raymodel")

	local wire_cd_lock = MakeWireCDLock( ply, trace.HitPos, model, Ang )
...
if (SERVER) then
	...
	function MakeWireCDRay( pl, Pos, Range, DefaultZero, Model, Ang )
	...
	duplicator.RegisterEntityClass("gmod_wire_cd_ray", MakeWireCDRay, "Pos", "Range", "DefaultZero", "Model", "Ang", "Vel", "aVel", "frozen")

	function MakeWireCDLock( pl, Pos, Model, Ang )
	...
	duplicator.RegisterEntityClass("gmod_wire_cd_lock", MakeWireCDLock, "Pos", "Model", "Ang", "Vel", "aVel", "frozen")
